...
export function mockEmailPayload(overrides: Partial<EmailPayloadDto> = {}): EmailPayloadDto {
  return {
...
describe('SpamFilterService', () => {
  ...
  it('no attachment and short body', () => {
    const payload = mockEmailPayload({ TextBody: 'hi', Attachments: [] });
  ...
  it('keyword variations', () => {
    const payloadUpper = mockEmailPayload({
      Subject: 'NEWSLETTER',
    ...
    const payloadMixedClean = mockEmailPayload({
      Subject: 'Job Application',
    ...
    const payloadMixedSpam = mockEmailPayload({
      Subject: 'Job Application',
